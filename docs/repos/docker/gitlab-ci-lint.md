...
editUrl: <https://gitlab.com/megabyte-labs/docker/ci-pipeline/gitlab-ci-lint/blob/master/docs/GUIDE.md>
...
packageGroup: false
packageName: '@megabytelabs/gitlab-ci-lint'
packageSlug: false
...
<div align="center">
  <center>
    <a href="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint">
      <img width="140" height="140" alt="GitLab CI Lint logo" src="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/raw/master/logo.png" />
    </a>
  </center>
</div>
<div align="center">
  <center><h1>GitLab CI Lint (A Linter for `.gitlab-ci.yml` Files)</h1></center>
  <center><h4 style="color: #18c3d1;">A <a href="https://megabyte.space">Megabyte Labs</a> Dockerfile project / Latest build includes GitLab CI Lint v1.1.0</h4></center>
</div>
...
<div align="center">
  <h4 align="center">
    ...
    <a href="https://hub.docker.com/r/megabytelabs/gitlab-ci-lint" title="DockerHub page for this project" target="_blank">
      <img src="https://gitlab.com/megabyte-labs/assets/-/raw/master/svg/dockerhub-image-solid.svg" />
    </a>
    <a href="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/blob/master/CONTRIBUTING.md" title="Learn about contributing" target="_blank">
    ...
    <a href="https://github.com/ProfessorManhattan/docker-gitlab-ci-lint" title="GitHub mirror" target="_blank">
      <img src="https://gitlab.com/megabyte-labs/assets/-/raw/master/svg/github-solid.svg" />
    </a>
    <a href="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint" title="GitLab repository" target="_blank">
      <img src="https://gitlab.com/megabyte-labs/assets/-/raw/master/svg/gitlab-solid.svg" />
    </a>
  </h4>
  <p align="center">
    <a href="https://hub.docker.com/repository/docker/megabytelabs/gitlab-ci-lint" target="_blank">
      <img alt="Version" src="https://img.shields.io/docker/v/megabytelabs/gitlab-ci-lint?logo=docker&logoColor=white&sort=date&style=flat" />
    </a>
    <a href="https://hub.docker.com/repository/docker/megabytelabs/gitlab-ci-lint" target="_blank">
      <img alt="DockerHub image size: GitLab CI Lint" src="https://img.shields.io/docker/image-size/megabytelabs/gitlab-ci-lint?logo=docker&sort=date&logoColor=white&style=flat">
    </a>
    <a href="https://hub.docker.com/repository/docker/megabytelabs/gitlab-ci-lint" target="_blank">
      <img alt="DockerHub pulls: GitLab CI Lint" src="https://img.shields.io/docker/pulls/megabytelabs/gitlab-ci-lint?logo=docker&logoColor=white&style=flat" />
    </a>
    <a href="https://hub.docker.com/repository/docker/megabytelabs/gitlab-ci-lint" target="_blank">
      <img alt="DockerHub stars: GitLab CI Lint" src="https://img.shields.io/docker/stars/megabytelabs/gitlab-ci-lint?logo=docker&logoColor=white&style=flat" />
    </a>
    <a href="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/commits/master" target="_blank">
      <img alt="GitLab pipeline status" src="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/badges/master/pipeline.svg?style=flat" />
    </a>
    <a href="https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/raw/master/LICENSE" target="_blank">
...
```shell
docker run -v ${PWD}:/work -w /work megabytelabs/gitlab-ci-lint:slim --help
```

This allows you to run GitLab CI Lint without installing it locally. This could be good for security since the application is within a container and also keeps your file system clean.

You can also add a bash alias to your `~/.bashrc` file so that you can run the GitLab CI Lint command at any time. To do this, add the following snippet to your `~/.bashrc` file (or `~/.bash_profile` if you are on macOS):

```shell
gitlab-ci-lint() {
  docker run -v ${PWD}:/work -w /work megabytelabs/gitlab-ci-lint:slim "$@"
}
```
...
docker build --pull -t "$DOCKERHUB_USERNAME/gitlab-ci-lint:latest" .
docker push "$DOCKERHUB_USERNAME/gitlab-ci-lint:latest"
```
...
export DOCKERHUB_PASSWORD=Your_DockerHub_Password_Here
docker login -u "$DOCKERHUB_USERNAME" -p "$DOCKERHUB_PASSWORD" docker.io
docker build -t "$DOCKERHUB_USERNAME/gitlab-ci-lint:latest" .
docker-slim build --tag $DOCKERHUB_USERNAME/gitlab-ci-lint:slim --http-probe=false --exec 'gitlab-ci-lint --version' --preserve-path-file 'paths.txt' $DOCKERHUB_USERNAME/gitlab-ci-lint:latest
docker push "$DOCKERHUB_USERNAME/gitlab-ci-lint:slim"
```

It may be possible to modify the DockerSlim command above to fix an issue or reduce the footprint even more than our command. You can modify the slim build command inline in the `package.json` file. However, running `bash .start.sh` will overwrite your changes in the `package.json` file. We detail a better way of modifying the `npm run build:slim` configuration in [CONTRIBUTING.md](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/blob/master/CONTRIBUTING.md).

If you come up with an improvement, please do [open a pull request](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/issues/new). And again, make sure you replace `DOCKERHUB_USERNAME` and `DOCKERHUB_PASSWORD` in the snippet above with your Docker Hub username and password. The commands in the snippet above will build the slim Docker image and upload it to [Docker Hub](https://hub.docker.com/) where it will be publicly accessible. You can see this logic being implemented as a [GitLab CI task here](https://gitlab.com/megabyte-labs/ci/gitlab-ci-templates/-/blob/master/dockerhub-slim.gitlab-ci.yml).
...
For more details, check out the [CONTRIBUTING.md](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/blob/master/CONTRIBUTING.md) file.

[![-----------------------------------------------------](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png)](#contributing)

## ➤ Contributing

Contributions, issues, and feature requests are welcome! Feel free to check the [issues page](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/issues). If you would like to contribute, please take a look at the [contributing guide](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/blob/master/CONTRIBUTING.md).
...
Copyright © 2021 [Megabyte LLC](https://megabyte.space). This project is [MIT](https://gitlab.com/megabyte-labs/dockerfile/ci-pipeline/gitlab-ci-lint/-/raw/master/LICENSE) licensed.
